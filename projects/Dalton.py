...
for i in range(10):
    ...
    for j in range(WIDTH // step + 2):
        create_peg(peg_x, peg_y, space, 'darkblue')
        if i == 9:
            create_segment((peg_x, peg_y + 50), (peg_x, HEIGHT), segment_thickness, space, 'red')
        peg_x += step
    peg_y += 0.5 * step

platforms = (L1, L2), (L2, L3), (L3, L4), (R1, R2), (R2, R3), (R3, R4)
for platform in platforms:
    create_segment(*platform, segment_thickness, space, 'green')
create_segment(B1, B2, 20, space, 'red')

balls = [([randrange(256) for i in range(3)], create_ball(space)) for j in range(600)]

while True:
    surface.fill(pg.Color('silver'))
